...
describe 'dotfiles::config' do
  ...
  it 'configure bash' do
    %w(bash_aliases bashrc).each do |item|
      expect(chef_run).to create_cookbook_file(item).with(
        source: item,
        path: "/home/got/.#{item}",
        owner: 'got',
        group: 'got'
  ...
  it 'configure terminator' do
    expect(chef_run).to create_directory('terminator').with(
      path: '/home/got/.config/terminator',
      owner: 'got',
      group: 'got',
      recursive: true
    )

    expect(chef_run).to create_template('config/terminator/config').with(
  ...
  it 'configure fluxbox' do
    expect(chef_run).to create_remote_directory('fluxbox').with(
      source: 'fluxbox',
      path: '/home/got/.fluxbox',
      owner: 'got',
      group: 'got'
    )
    expect(chef_run).to create_template('fluxbox/init').with(
  ...
  it 'configure conkyrc' do
    expect(chef_run).to create_remote_directory('conkyrc').with(
      source: 'conkyrc',
    ...
    %w(computer general network system).each do |item|
      expect(chef_run).to create_template("conkyrc/#{item}").with(
